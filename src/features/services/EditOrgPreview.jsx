  ...
  return (
    <div className="max-w-2xl mx-auto py-4 px-2 md:px-0 animate-in fade-in slide-in-from-right-16 duration-700 ease-out">
      <div className="flex items-center justify-between mb-8">
        ...
        <Button variant="ghost" onClick={() => navigate('/edit_org', { state: { formData, step: 2 } })} className="text-slate-400 hover:text-white hover:bg-slate-800 rounded-xl">
        ...
        <div ref={captureRef} className="bg-gradient-to-b from-slate-900 to-slate-950 rounded-2xl p-8 border border-slate-800 shadow-2xl relative overflow-hidden text-slate-200">
          ...
          <div className="mb-8 relative z-10">
            ...
            <ul className="text-sm flex flex-col gap-2.5 list-none pl-0 font-medium">
              {formData.changeInfo && <li className="flex justify-between items-center bg-slate-800/60 p-3 rounded-xl border border-slate-700 gap-4"><span className="flex items-start sm:items-center gap-2 text-white flex-1 min-w-0"><CheckCircle size={18} weight="fill" className="text-amber-500 shrink-0 mt-0.5 sm:mt-0" /> <span className="whitespace-nowrap">เปลี่ยนข้อมูล {formData.orgType === 'FAMILY' ? 'Family' : 'Gang'}</span></span> <span className="font-mono text-amber-400 shrink-0">$500,000</span></li>}
              {formData.editTexture && <li className="flex justify-between items-center bg-slate-800/60 p-3 rounded-xl border border-slate-700 gap-4"><span className="flex items-start sm:items-center gap-2 text-white flex-1 min-w-0"><CheckCircle size={18} weight="fill" className="text-amber-500 shrink-0 mt-0.5 sm:mt-0" /> <span className="whitespace-nowrap">แก้ไข Texture เสื้อผ้า <span className="text-slate-400 text-xs ml-1">x{formData.textureCount}</span></span></span> <span className="font-mono text-amber-400 shrink-0">${(500000 * Math.max(1, formData.textureCount)).toLocaleString()}</span></li>}
              {formData.addCloth && <li className="flex justify-between items-center bg-slate-800/60 p-3 rounded-xl border border-slate-700 gap-4"><span className="flex items-start sm:items-center gap-2 text-white flex-1 min-w-0"><CheckCircle size={18} weight="fill" className="text-amber-500 shrink-0 mt-0.5 sm:mt-0" /> <span className="whitespace-nowrap">ลงชุดเพิ่ม <span className="text-slate-400 text-xs ml-1">x{formData.textureCount}</span></span></span> <span className="font-mono text-amber-400 shrink-0">${(500000 * Math.max(1, formData.textureCount)).toLocaleString()}</span></li>}
              {formData.bulkChange && <li className="flex justify-between items-center bg-slate-800/60 p-3 rounded-xl border border-slate-700 gap-4"><span className="flex items-start sm:items-center gap-2 text-white flex-1 min-w-0"><CheckCircle size={18} weight="fill" className="text-amber-500 shrink-0 mt-0.5 sm:mt-0" /> <span className="whitespace-nowrap">เหมาเปลี่ยนข้อมูล {formData.orgType === 'FAMILY' ? 'Family' : 'Gang'}</span></span> <span className="font-mono text-amber-400 shrink-0">$1,500,000</span></li>}
              {formData.addAccessory && <li className="flex justify-between items-center bg-slate-800/60 p-3 rounded-xl border border-slate-700 gap-4"><span className="flex items-start sm:items-center gap-2 text-white flex-1 min-w-0"><CheckCircle size={18} weight="fill" className="text-amber-500 shrink-0 mt-0.5 sm:mt-0" /> <span className="whitespace-nowrap">ลง Accessories Adons เสริม</span></span> <span className="font-mono text-amber-400 shrink-0">$1,000,000</span></li>}
            </ul>
          </div>

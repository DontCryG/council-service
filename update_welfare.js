const fs = require('fs');
const path = 'c:/Users/DontCry/Downloads/COUNCIL_SERVICE_V2/src/features/services/Welfare.jsx';

let content = fs.readFileSync(path, 'utf8');

// Add imports if missing
if (!content.includes('Users,')) {
    content = content.replace('import { PaperPlaneTilt, Plus, Trash, Gift, Car, Shield, Buildings }', 'import { PaperPlaneTilt, Plus, Trash, Gift, Car, Shield, Buildings, Users, House, Sword }');
}
...
const newRender = \  return (
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500 max-w-[1200px] mx-auto">
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-2xl font-bold text-white">????????????????? Gang / Family</h1>
          <p className="text-slate-400">????????????????????????????????? GANG / FAMILY</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 items-start">
        ...
        <div className="bg-white rounded-[24px] p-8 shadow-sm">
          <form onSubmit={handleSubmit} className="space-y-8">
            ...
            <div className="space-y-3">
              <label className="text-[13px] font-bold text-slate-400 tracking-wide">1. ??????????????????</label>
              <div className="grid grid-cols-2 gap-4">
                <button
                  type="button"
                  className={\\\py-4 rounded-xl font-bold flex flex-col sm:flex-row items-center justify-center gap-2 transition-all border-2 \\\\}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="space-y-3">
                <label className="text-[13px] font-bold text-slate-400 tracking-wide">2. ???? GANG / FAMILY</label>
                <input 
              ...
              <div className="space-y-3">
                <label className="text-[13px] font-bold text-slate-400 tracking-wide">3. ?????????????????</label>
                <input 
            ...
            <div className="space-y-3">
              <label className="text-[13px] font-bold text-slate-400 tracking-wide">4. ???????????????????????</label>
              <div className="grid grid-cols-1 xl:grid-cols-2 gap-4">
                ...
                <div className="bg-white border-2 border-slate-100 rounded-2xl p-6 flex flex-col min-h-[240px]">
                  ...
                  <div className="flex-1 flex flex-col">
                    <label className="text-[10px] font-bold text-slate-400 mb-2">??????????????</label>
                    <textarea 
            ...
            <Button type="submit" className="w-full h-14 text-base shadow-lg shadow-blue-500/20" isLoading={isSubmitting}>
        ...
        <div className="sticky top-24">
          ...
          <div ref={captureRef} className="bg-white rounded-xl p-8 sm:p-12 shadow-2xl relative overflow-hidden text-slate-800 font-sans border-t-[6px] border-amber-500">
            ...
            <div className="flex justify-between items-start mb-10 relative z-10">
              ...
              <div className="text-right">
                <h2 className="text-2xl font-black text-slate-900 tracking-tight">?????????</h2>
                <p className="text-[10px] font-bold text-amber-600 tracking-widest uppercase">SUPREME COUNCIL OFFICE</p>
              </div>
            </div>

            <div className="text-center mb-10 relative z-10">
              <h1 className="text-2xl font-black text-slate-800 inline-block border-b-2 border-slate-800 pb-1 mb-2">???????????????????????</h1>
              <p className="text-xs text-slate-500 font-bold">?????????????: CS-WELFARE-0512</p>
            </div>

            <div className="mb-8 leading-relaxed text-slate-800 text-sm font-medium relative z-10">
              <div className="flex items-end gap-2 mb-4">
                <span className="font-bold whitespace-nowrap text-base">????????</span>
              ...
              <div className="flex items-end gap-2">
                ...
                <span className="font-bold whitespace-nowrap ml-2 text-base">????????????????????????????????:</span>
              </div>
            </div>

            <div className="bg-slate-50 rounded-xl p-6 border border-slate-100 mb-8 relative z-10">
              <h3 className="text-[10px] font-bold text-slate-400 tracking-wider mb-4 uppercase">???????????????????????? (WELFARE ITEMS)</h3>
              <div className="space-y-3 bg-white p-4 rounded-lg border border-slate-100 shadow-sm">
                ...
                {vehicles.map((v, i) => (
                  <div key={i} className="flex items-center gap-3 text-slate-700 font-bold text-sm">
                     <span className="text-amber-500 text-lg leading-none">›</span> ?????: {v.model || '...'} {v.plate ? \\\(\\\)\\\ : ''}
                  </div>
                ))}
                {formData.otherWelfare && (
                  <div className="flex items-center gap-3 text-slate-700 font-bold text-sm">
                     <span className="text-amber-500 text-lg leading-none">›</span> ?????: {formData.otherWelfare}
                  </div>
                )}
            ...
            <div className="border-l-4 border-amber-500 pl-6 mb-8 text-slate-600 text-[13px] leading-relaxed space-y-4 relative z-10">
              <p><strong className="text-slate-800">??????????????????:</strong> ?????????????????????????????????????? ??????? ????????? ?????? ???????????????????????? ????????????????????????????????????????????????????????? ???????????????????? ?</p>
              <p>???????????????????????????????????????? <strong className="text-slate-800">??????????? 3 ?????</strong> ???????????????????????????????????????????????? ??????????????????????????</p>
              <p>??????????????????????? ???????????????????????????????????? ??????????????????????????? ???????? ?????????????? ????????????????? ???????????????????????? ???????????????????????????????????????</p>
            </div>
            ...
            <div className="flex justify-between items-end relative z-10">
              <div>
                <div className="text-[10px] text-slate-400 font-bold tracking-wider uppercase mb-1">??????????</div>
...
const newFileContent = content.substring(0, renderStart) + newRender;
fs.writeFileSync(path, newFileContent, 'utf8');
console.log('Successfully updated Welfare.jsx');
